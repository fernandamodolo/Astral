...
<?php
   ...
   session_start();
    ...
    {
        echo("vocÃª nao estÃ¡ logado, faÃ§a o seu login agora!");
...
//     echo "<form action='altera_dados.php?' class='cad_pessoa' method='POST'>";
     ?>
        <form action='altera_dados.php?' name="alt" class='cad_pessoa' method='POST' >
           <label align="left">Nome:<input type='text' class='insere' name='nome' value='<?php echo $linha[1];?>'  autocomplete='off'></label>
            ...
            <label align="left">Telefone<input type='text' class='insere' name='tel' value='<?php echo $linha[3];?>'  autocomplete='off' onkeypress="mascara5(this, '## #####-####')"  pattern="[0-9]{2} [0-9]{5,5}-[0-9]{5,4}" onblur="return Onlynumbers(event)" maxlength="15"  ></label>
          
                      
            <label align="left">Email<input type='email' class='insere' name='email' value='<?php echo $linha[4];?>' autocomplete='off' readonly="readonly"></label>
           
            <label align="left"> Nova Senha <input  oninput="forca_senha()" id="senha_alt" type="password" class='insere forca' class='forca' name="senha"  value = '' autocomplete='off' minlength = "8" maxlength="14" size="14"></label>
...
//         var auxEspecial = 0;
        ...
        if(forca == 3)
        {
            id_forca.classList.add('forte');
             ...
             <form action="pag_usuario.php" method="post">
